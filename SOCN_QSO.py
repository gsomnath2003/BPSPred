...
# Schneider-Wrede Distance Matrix
...
def GetAAComposition(ProteinSequence):
    ...
    for i in AALetter:
        Result[i] = float(ProteinSequence.count(i)) / LengthSequence
...
def GetQuasiSequenceOrder(
    ...
    for aa in AALetter:
        ...
        Result[prefix + '_' + aa] = AAC[aa] / temp

    # Lag descriptors
    for index in range(maxlag):

        Result[prefix + '_lag' + str(index + 1)] = (
            weight * rightpart[index] / temp
        )

    return Result

# Calculate Descriptors
def socn_qso_desc_calculator(sequences):
    all_results = []
    ...
    for seq in sequences:
        ...
        # SOCN - Schneider-Wrede
        for lag in range(1, maxlag + 1):

            result['SOCNSW_lag' + str(lag)] = (
                GetSequenceOrderCouplingNumber(
                    seq,
                    d=lag,
        ...
        # Add descriptors
        result.update(qso_sw)
        result.update(qso_gm)

        all_results.append(result)

    # Save Output
    df = pd.DataFrame(all_results, index=sequences)
    return df
